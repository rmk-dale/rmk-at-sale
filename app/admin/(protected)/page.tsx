    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="text-center py-24 bg-surface rounded-2xl border border-border">
          ...
          <Link href="/admin/products/new" className="inline-flex bg-zinc-900 text-white px-6 py-3 rounded-xl font-medium hover:bg-zinc-700 transition-colors">
            Add your first item
          </Link>
        </div>
      ) : (
            ...
            <tbody>
              {products.map((product) => (
                <tr key={product._id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-5 py-3 font-mono text-xs text-zinc-600">{product._id}</td>
                  <td className="px-5 py-3 text-zinc-900">{product.description}</td>
                  <td className="px-5 py-3 text-zinc-900">${product.price.toFixed(2)}</td>
                  <td className="px-5 py-3">
                    <span
                  ...
                  <td className="px-5 py-3 text-right">
                    <Link href={`/admin/products/${product._id}`} className="text-zinc-500 hover:text-zinc-900 text-sm font-medium">
                      Edit
                    </Link>
                  </td>
                </tr>
