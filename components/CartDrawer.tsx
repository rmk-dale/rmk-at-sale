      ...
      <div className="absolute top-0 right-0 h-full w-full max-w-md bg-surface border-l border-border flex flex-col animate-in slide-in-from-right duration-300">
        <div className="flex items-center justify-between px-6 h-16 border-b border-border">
          <h2 className="text-lg font-semibold text-zinc-900">
            Your cart {items.length > 0 && <span className="text-zinc-400 font-normal">({items.length})</span>}
          </h2>
          <button
            onClick={closeCart}
            ...
            <div className="space-y-5">
              {items.map((item) => (
                <div key={item.id} className="flex items-center gap-4">
                  <div className="w-16 h-16 bg-zinc-50 rounded-xl border border-border flex-shrink-0" />
        ...
        {items.length > 0 && (
          <div className="border-t border-border px-6 py-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-lg font-semibold text-zinc-900">${getTotal().toFixed(2)}</span>
            </div>
            <Link
              href="/cart"
              onClick={closeCart}
