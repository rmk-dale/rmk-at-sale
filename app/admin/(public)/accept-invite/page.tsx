...
function AcceptInviteForm() {
  ...
  const [step, setStep] = useState<'PASSWORD' | 'ENROLL_2FA' | 'BACKUP_CODES'>('PASSWORD');
  ...
  if (!id || !token) {
    return <p className="text-zinc-500 text-sm">This invite link is missing its id or token.</p>;
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 py-12">
      <div className="w-full max-w-md bg-surface border border-border rounded-2xl p-8">
        <h1 className="text-xl font-semibold text-zinc-900 mb-1">Set up your admin account</h1>

        {step === 'PASSWORD' && (
          <form onSubmit={handleSetPassword} className="space-y-4 mt-6">
            <p className="text-sm text-zinc-500 mb-2">Choose a password (at least 10 characters).</p>
            <div>
          ...
          <form onSubmit={handleConfirmCode} className="space-y-4 mt-6">
            ...
            {qrCodeDataUrl && (
              <div className="flex justify-center py-2">
                <Image src={qrCodeDataUrl} alt="Two-factor QR code" width={180} height={180} unoptimized />
              </div>
            )}
            <details className="text-xs text-zinc-400">
              <summary className="cursor-pointer">Can&apos;t scan? Enter this URL manually</summary>
              <p className="break-all mt-1">{otpAuthUrl}</p>
            </details>
            <div>
              <label className="block text-sm font-medium text-zinc-600 mb-2">6-digit code</label>
              <input
                required
                value={code}
        ...
        {step === 'BACKUP_CODES' && (
          <div className="space-y-4 mt-6">
            <p className="text-sm text-zinc-500">
              Save these one-time backup codes somewhere safe. Each one can be used once to sign in if you lose access to your authenticator app — they won&apos;t be shown again.
            </p>
