...
import { Minus, Plus, Trash2, ArrowRight, CheckCircle2, Lock } from 'lucide-react';
...
export default function CartPage() {
  ...
  const [step, setStep] = useState<'CART' | 'EMAIL' | 'OTP' | 'SUCCESS'>('CART');
    ...
    return (
      <div className="max-w-2xl mx-auto px-6 py-24 text-center">
        <div className="bg-surface border border-border p-12 rounded-2xl animate-in zoom-in slide-in-from-bottom-8 duration-500">
          ...
          <h1 className="text-3xl font-semibold mb-4 text-zinc-900">Order confirmed</h1>
          <p className="text-zinc-500 text-lg mb-8">
            Your receipt has been sent to <span className="text-zinc-900 font-medium">{email}</span>. Thank you for your purchase.
          </p>
          <Link href="/" className="inline-flex bg-zinc-900 text-white px-8 py-3 rounded-xl font-medium hover:bg-zinc-700 transition-colors">
            Continue shopping
          </Link>
        </div>
      </div>
      ...
      {items.length === 0 ? (
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-4">
            {items.map((item) => (
              <div key={item.id} className="flex flex-col sm:flex-row items-center gap-6 bg-surface p-5 rounded-2xl border border-border relative">
                <div className="w-20 h-20 bg-zinc-50 rounded-xl flex-shrink-0 border border-border" />
                <div className="flex-grow">
                  <h3 className="text-base font-medium text-zinc-900 mb-1">{item.name}</h3>
                  <p className="text-zinc-500 font-medium text-sm">${item.price.toFixed(2)}</p>
                </div>

                <div className="flex items-center gap-4 bg-zinc-50 rounded-full px-4 py-2 border border-border">
                  <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-1 text-zinc-600 hover:text-zinc-900 transition-colors">
                    <Minus className="w-4 h-4" />
                  </button>
                  <span className="w-4 text-center font-medium text-zinc-900">{item.quantity}</span>
                  <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-1 text-zinc-600 hover:text-zinc-900 transition-colors">
          ...
          <div className="lg:col-span-1">
            <div className="bg-surface border border-border rounded-2xl p-8 sticky top-24">
              ...
              <div className="flex justify-between items-center mb-6">
                ...
                <span className="text-zinc-900 font-semibold text-2xl">${getTotal().toFixed(2)}</span>
              </div>
              ...
              {step === 'OTP' && (
                <form onSubmit={handleVerifyAndCheckout} className="space-y-4 animate-in slide-in-from-right-4 duration-300">
                  <div>
                    <label className="block text-sm font-medium text-zinc-600 mb-2">6-digit code sent to {email}</label>
                    <input
                      type="text"
                      required
                      maxLength={6}
