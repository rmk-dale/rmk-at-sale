...
export default function AdminsPage() {
  ...
  const updateAdmin = async (id: string, body: { role?: string; status?: string }) => {
    setActionError('');
  ...
  if (forbidden) {
    return <p className="text-zinc-500">Owner access is required to manage admins.</p>;
  }
  ...
  return (
    <div className="max-w-4xl">
      ...
      <form onSubmit={handleInvite} className="bg-surface border border-border rounded-2xl p-6 mb-8">
        ...
        {inviteSuccess && <p className="text-emerald-600 text-sm mb-3">{inviteSuccess}</p>}
        <button
          disabled={inviting}
          type="submit"
            ...
            <tbody>
              {admins.map((a) => (
                <tr key={a.id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-5 py-3 text-zinc-600">{a.twoFactorEnabled ? 'Enabled' : '—'}</td>
                  <td className="px-5 py-3 text-right">
                    {a.status === 'disabled' ? (
                      ...
                      <button
                        onClick={() => updateAdmin(a.id, { status: 'disabled' })}
